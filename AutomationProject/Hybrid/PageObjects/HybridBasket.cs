...
namespace AutomationProject.Hybrid.PageObjects
{
    internal class HybridBasket : HybridBase
    {
        ...
        #region Methods for HybridBase

        public IRestResponse AddToBasket(int idProduct, int qty, int ipa, string staticToken)
        {
            // Defines request URL
            var request = new RestRequest($"/index.php?rand={Rand}", Method.POST);

            // Constants for body request
